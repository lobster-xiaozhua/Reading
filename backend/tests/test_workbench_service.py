    ...
    async def test_word_trend_with_chapters(self, svc, db_session):
        ...
        db_session.add(
            Chapter(
                ...
                word_count=1000,
                status="published",
                published_at=today_ts - 86400000,
            )
        )
        db_session.add(
            Chapter(
...
class TestWorkbenchOverviews:
    ...
    async def test_overviews_with_data(self, svc, db_session):
        ...
        db_session.add(
            RewardRecord(reader_id=1, novel_id=1, type="tip", amount=100, created_at=today_ts)
        )
        db_session.add(
            Comment(novel_id=1, reader_id=1, content="好文", status=1, created_at=today_ts)
        )
        await db_session.flush()
