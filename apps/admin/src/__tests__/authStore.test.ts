...
function mockFetch(data: unknown) {
  vi.stubGlobal('fetch', vi.fn().mockResolvedValue({
    ok: true, status: 200, json: () => Promise.resolve({ code: 0, message: 'ok', data }),
  }));
}

describe('authStore', () => {
  beforeEach(() => {
    useAuthStore.setState({ token: null, user: null, expiresAt: 0, refreshToken: null });
    vi.restoreAllMocks();
  });
  ...
  it('login sets token and user', async () => {
    mockFetch({ token: 'token123', user: { id: '1', username: 'admin', nickname: 'Admin', roles: ['super-admin'], permissions: [], email: '', lastLoginAt: 0, enabled: true }, expiresAt: 3600000, refreshToken: 'refresh123' });
  ...
  it('hasPermission returns true for super-admin', () => {
    useAuthStore.setState({ user: { id: '1', roles: ['super-admin'], permissions: [] } as any });
    expect(useAuthStore.getState().hasPermission('novel.create')).toBe(true);
  });

  it('hasPermission checks user permissions', () => {
    useAuthStore.setState({ user: { id: '1', roles: ['content-admin'], permissions: ['novel.list', 'novel.edit'] } as any });
  ...
  it('hasRole checks user roles', () => {
    useAuthStore.setState({ user: { id: '1', roles: ['content-admin', 'auditor'], permissions: [] } as any });
