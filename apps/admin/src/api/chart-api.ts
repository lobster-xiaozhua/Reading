...
export async function fetchBasicChartData() {
  ...
  return {
    lineData: (data.data as { month: string; value: number; type: string }[]) || [],
    columnData: (data.data as { day: string; value: number; type: string }[]) || [],
    pieData: (data.data as { type: string; value: number }[]) || [],
    areaData: (data.data as { date: string; pv: number; uv: number }[]) || [],
    heatmapData: (data.data as { day: number; hour: number; value: number }[]) || [],
    gaugeValue: 0.78,
  };
}
