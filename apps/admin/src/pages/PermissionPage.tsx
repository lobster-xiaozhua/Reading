...
export default function PermissionPage() {
  ...
  const [selectedRoleKey, setSelectedRoleKey] = useState<AdminRole | null>(
    null,
  );
  ...
  const handleSavePermissions = async () => {
    ...
    setSaving(true);
    try {
      const perms = checkedKeys.filter(
        (k): k is Permission =>
          typeof k === "string" && !k.startsWith("module:"),
      ) as Permission[];
      const res = await updateRolePermissions(currentRole.key, perms);
      if (res.success) {
        message.success(
          t("permission:message.updated", { role: currentRole.name }),
        );
  ...
  const selectedCount = checkedKeys.filter(
        ...
        <div className="pp-layout">
                        ...
                        <div className="pp-item__header">
                          <Space>
                            ...
                            {role.builtin && (
                              <Tag
                                color="default"
                                className="pp-tree-node__tag"
                              >
                                {role.key === "super-admin" ? (
                                  <>
                                    <LockOutlined /> {t("permission:builtin")}
                                  </>
                                ) : (
          ...
          <div className="pp-content-area">
            {status === "loading" ? (
              ...
              <Card>
                <Empty
                  description={t("permission:emptySelect")}
                  className="pp-empty"
                />
              </Card>
            ) : (
                ...
                >
                  {editingMeta ? (
                    ...
                    <Descriptions column={2} size="small">
                      ...
                      <Descriptions.Item label={t("permission:field.roleName")}>
                        {currentRole.name}
                      </Descriptions.Item>
                      <Descriptions.Item
                        label={t("permission:field.dataScope")}
                      >
                        <Tag
                          color={
                            currentRole.dataScope === "all"
                              ? "success"
                              : currentRole.dataScope === "department"
                                ? "processing"
                                : "default"
                          }
                        >
                      ...
                      <Descriptions.Item
                        label={t("permission:field.userCount")}
                      >
                        {t("permission:userCount", {
                          count: currentRole.userCount,
                        })}
                      </Descriptions.Item>
                      <Descriptions.Item
                        label={t("permission:field.description")}
                        span={2}
                      >
                ...
                <Card
                  title={
                    <Space>
                      ...
                      {isReadonly && (
                        <Tag color="warning">
                          <LockOutlined /> {t("permission:readonly")}
                        </Tag>
                      )}
                    </Space>
                  }
