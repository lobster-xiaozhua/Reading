// 把 src/tailwind.ts 编译为 dist/tailwind.js / .cjs / .d.ts
// 简化方案：直接生成 JS（不含类型导入），并手写 .d.ts
...
// ESM 输出（直接对象字面量，避免依赖 tailwindcss 类型）
const esm = `/* Auto-generated from src/tailwind.ts. 不要手动编辑。 */
const config = {
  ...
  theme: {
    extend: {
      ...
      transitionDuration: {
        ...
        normal: 'var(--dur-normal)', slow: 'var(--dur-slow)',
...
const cjs = esm.replace('export default config;', 'module.exports = config;');

const dts = `import type { Config } from 'tailwindcss';
declare const config: Config;
export default config;
`;

writeFileSync(join(outDir, 'tailwind.js'), esm);
writeFileSync(join(outDir, 'tailwind.cjs'), cjs);
writeFileSync(join(outDir, 'tailwind.d.ts'), dts);
console.log('[tokens:tailwind] emitted tailwind.js/.cjs/.d.ts');
