...
export const BContentReview = forwardRef<HTMLDivElement, BContentReviewProps>(
  function BContentReview(
    { item, items = [], mode = 'single', history = [], onApprove, onRevise, onReject },
    ref,
  ) {
      ...
      <div ref={ref} className="b-content-review">
            ...
            currentItems.map((it) => {
              ...
              <div key={it.id} style={{ marginBottom: 'var(--space-4)' }}>
                <h3 style={{ fontSize: 'var(--font-size-h3, 20px)', fontWeight: 600, marginBottom: 'var(--space-2)' }}>
                  {it.title}
                  <span style={{ fontSize: 'var(--font-size-body, 14px)', color: 'var(--color-text-secondary)', marginLeft: 'var(--space-2)' }}>
                    作者：{it.author}
                  </span>
                </h3>
                {tagWords.length > 0 && (
              ...
              items={history.map((h) => ({
                ...
                children: (
                  <div>
                    ...
                    {h.comment && (
                      <p style={{ marginTop: 'var(--space-1)', color: 'var(--color-text-primary)' }}>{h.comment}</p>
                    )}
                  </div>
                ),
              }))}
        ...
        <Card title="审核操作">
          <div style={{ display: 'flex', flexDirection: 'column', gap: 'var(--space-3)' }}>
            ...
            />
            <Space>
              <Button type="primary" loading={submitting} onClick={handleApprove} disabled={currentIds.length === 0}>
                通过
              </Button>
              <Button loading={submitting} onClick={handleRevise} disabled={currentIds.length === 0}>
                待修改
              </Button>
              <Button danger loading={submitting} onClick={handleReject} disabled={!canReject || currentIds.length === 0}>
                驳回
              </Button>
            </Space>
          </div>
