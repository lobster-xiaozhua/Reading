 ...
 */
export function createSensitivePlugin(words: SensitiveWord[]) {
  // 预处理：过滤空词，转小写缓存
      ...
      return [
        new Plugin<SensitiveState>({
          ...
          state: {
            ...
            apply(tr: Transaction, value: SensitiveState, _oldState, newState) {
              // 位置随事务映射，保证光标移动/删除时装饰跟随
              const mapped = value.set.map(tr.mapping, tr.doc) as DecorationSet;
