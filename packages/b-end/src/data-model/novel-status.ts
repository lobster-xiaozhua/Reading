...
/** 校验状态转换是否合法 */
export function canTransitionNovel(from: BNovelStatus, to: BNovelStatus): boolean {
  const allowed = NOVEL_TRANSITIONS[from] ?? [];
  return allowed.includes(to);
}

/** 执行状态转换，非法则抛错 */
export function transitionNovel(from: BNovelStatus, to: BNovelStatus): BNovelStatus {
