...
function toStepStatus(s: FlowNodeStatus): 'wait' | 'process' | 'finish' | 'error' {
  switch (s) {
    case 'approved':
      return 'finish';
    case 'processing':
...
function statusIcon(s: FlowNodeStatus): React.ReactNode {
  switch (s) {
    case 'approved':
      return <CheckCircleFilled style={{ color: 'var(--color-feedback-success)' }} />;
    case 'processing':
      return <LoadingOutlined style={{ color: 'var(--color-brand)' }} />;
    case 'rejected':
      return <CloseCircleFilled style={{ color: 'var(--color-feedback-error)' }} />;
    case 'waiting':
    default:
      return <ClockCircleOutlined style={{ color: 'var(--color-text-tertiary)' }} />;
  }
}
 ...
 */
export function SettlementFlow({ nodes, current, className }: SettlementFlowProps) {
    ...
    <div className={className} style={{ padding: 'var(--space-4)' }}>
      <Steps
        ...
        items={list.map((n, idx) => ({
          ...
          status: idx === activeIndex && n.status === 'processing' ? 'process' : toStepStatus(n.status),
