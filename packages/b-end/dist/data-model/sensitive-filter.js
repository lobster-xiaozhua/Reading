...
export const SENSITIVE_LEVEL_META = {
    1: {
...
/** 取等级对应的过滤动作 */
export function getFilterAction(level) {
 ...
 */
export function shouldBlockSave(hits) {
 ...
 * 是否存在需人工审核的命中（二级）。
 */
export function hasRequireAudit(hits) {
 ...
 * @param hits 命中清单
 */
export function splitContentBySensitive(content, hits) {
    if (hits.length === 0)
        return [{ text: content, isHit: false }];
