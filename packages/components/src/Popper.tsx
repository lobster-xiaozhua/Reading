 ...
 */
export function usePopper({ placement = 'top', offset = 8, open }: PopperOptions) {
  const triggerRef = useRef<HTMLDivElement | null>(null);
  const floatRef = useRef<HTMLDivElement | null>(null);
  const [floatStyle, setFloatStyle] = useState<React.CSSProperties>({ visibility: 'hidden' });
  ...
  useEffect(() => {
    ...
    let top = 0;
    let left = 0;
    const [side, align] = [placement.replace(/(Start|End)$/, ''), placement.match(/(Start|End)$/)?.[1] ?? 'Center'];
