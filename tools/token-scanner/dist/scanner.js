...
function isIgnored(file, root, ignore) {
    ...
    return ignore.some((p) => {
        ...
        if (p.startsWith('**/') && p.endsWith('/**')) {
            ...
            if (rel.includes('/' + mid + '/') || rel.startsWith(mid + '/'))
                return true;
        }
        // glob 风格：**/*.ext 匹配任意路径下的该扩展名文件
        if (p.startsWith('**/*.')) {
            const suffix = p.slice(3); // *.ext
            if (rel.endsWith(suffix.slice(1)))
                return true; // .ext
        }
        return false;
    });
}
function isTokenCssFile(file) {
    // 令牌定义文件本身允许写色值
            ...
            if (!isTokenDef && (ext === '.ts' || ext === '.tsx' || ext === '.jsx' || ext === '.css')) {
                ...
                while ((m = PX_RE.exec(line)) !== null) {
                    ...
                    if (val > 0 && val <= 200 && !ALLOWED_SPACING_PX.has(val)) {
                        ...
                        if (/(font-size|line-height|border-radius|border-width|width|height|min-width|max-width)/i.test(line) ||
